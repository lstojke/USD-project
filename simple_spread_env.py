...
class PettingZooEnv(BaseEnv):
    ...
    def reset(self) -> np.ndarray:
        ...
        if self._replay_path is not None:
            before_training = f"simple_spread/qmix/trained/{str(uuid.uuid4())}.mp4"
                ...
                self._observation_space = gym.spaces.Dict(
                    {
                        ...
                        "global_state": gym.spaces.Box(
                            ...
                            shape=(
                                4 * self._num_agents
                                + 2 * self._num_landmarks
                                + 2 * self._num_agents * (self._num_agents - 1),
                            ),
                            dtype=np.float32,
                        ),
    ...
    def _process_obs(self, obs: "torch.Tensor") -> np.ndarray:  # noqa
        ...
        ret["global_state"] = np.concatenate(
            [
                obs[
                    0, 2 : -(self._num_agents - 1) * 2
                ],  # all agents' position + all landmarks' position
                obs[:, 0:2].flatten(),  # all agents' velocity
                obs[
                    :, -(self._num_agents - 1) * 2 :
                ].flatten(),  # all agents' communication
            ]
        )
        ...
        ret["agent_alone_state"] = np.concatenate(
            [
                obs[
                    :, 0 : (4 + self._num_agents * 2)
                ],  # agent itself's state + landmarks' position
        ...
        ret["agent_alone_padding_state"] = np.concatenate(
            [
                ...
                np.zeros(
                    (self._num_agents, (self._num_agents - 1) * 2), np.float32
                ),  # Other agents' position(0-padding)
                obs[:, -(self._num_agents - 1) * 2 :],  # communication
            ],
            1,
        )
